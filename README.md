...
- **세션이 끊겨도 이어짐**: 작업 맥락(결정·설계 배경)을 `docs/context/`에 남겨, 새 세션·다른 날에도 Claude가 이어받는다. (`claude/skills/project-context`)
- **자기 측정·안전 가드**: 어떤 스킬/커맨드를 실제 쓰는지 로그로 쌓아 안 쓰는 건 정리(`~/.claude/harness-usage.log`). 안전 규칙(개인계정 커밋)은 훅으로 강제(`claude/hooks/`).
...
근데 단순 동기화에서 멈추지 않는다 — **반복 작업을 Claude가 스스로 감지해 자동 처리하고(하네스), 내가 직접 커스텀 도구(스킬·커맨드·스크립트·훅)를 만들어 쌓는다.** 다른 dotfiles/설정 모음과 달리 *쓸수록 내 워크플로에 맞춰 스스로 굴러간다*. (자세히는 위 [뭐가 특별한가](#뭐가-특별한가-하네스))
