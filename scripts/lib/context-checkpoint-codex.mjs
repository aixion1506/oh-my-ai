...
const FILE_MUTATION_TOOLS = new Set(["apply_patch"]);

export function handleCodexCheckpointHook(eventName, rawInput, options = {}) {
  const event = parseEvent(rawInput);
  if (!event) {
    return unavailableOutput(eventName, "hook_payload_unavailable");
  }
  if (eventName === "UserPromptSubmit" && !isExplicitHandoffInvocation(event.prompt)) {
    return {};
  }
  if (event.hook_event_name !== eventName) {
    return unavailableOutput(eventName, "hook_payload_unavailable");
  }
  ...
  if (eventName === "UserPromptSubmit") {
    return handleUserPromptSubmit(event, options);
  }
  if (eventName === "SessionEnd") {
    // Codex SessionEnd is advisory and cannot steer the closing thread.
    // Activity is persisted at PostToolUse/UserPromptSubmit time.
    checkpointStatus({
      cwd: event.cwd,
      env: options.env,
    });
...
function handleUserPromptSubmit(event, options) {
  const result = recordCheckpointActivity({
    cwd: event.cwd,
    runtime: "codex",
    sessionId: event.session_id,
    eventId: event.turn_id,
    signalKind: "explicit_handoff_boundary",
    env: options.env,
  });
  return result.availability === "available"
    ? {}
    : unavailableOutput("UserPromptSubmit", result.reason_code);
}
...
function isExplicitHandoffInvocation(prompt) {
  if (typeof prompt !== "string" || prompt.length > 64 * 1024) return false;
  return /^\s*(?:\$|\/)handoff(?:\s|$)/i.test(prompt);
}
